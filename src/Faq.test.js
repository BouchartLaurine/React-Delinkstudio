...
describe("Composant FAQ", () => {
  test("affiche toutes les questions", () => {
    ...
    expect(
      screen.getByText(/Quels soins après un tatouage ?/i),
    ).toBeInTheDocument();
    expect(screen.getByText(/Faut-il un acompte ?/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Est-ce que les mineurs peuvent se faire tatouer ?/i),
    ).toBeInTheDocument();
  ...
  test("affiche la réponse quand on clique sur une question et la masque quand on reclique", () => {
    ...
    expect(
      screen.queryByText(/Le ressenti de la douleur varie/i),
    ).not.toBeInTheDocument();

    // Clique sur la question — la réponse doit apparaître
    fireEvent.click(question);
    expect(
      screen.getByText(/Le ressenti de la douleur varie/i),
    ).toBeInTheDocument();
